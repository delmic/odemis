...
class MockComponent(model.HwComponent):
    ...
    """
    def __init__(self, name, role, _realcls, parent=None, children=None, _vas=None, daemon=None, dependencies=None, **kwargs):
        """
        _realcls (class): the class we pretend to be
        _vas (list of string): a list of mock vigilant attributes to create
        """
        model.HwComponent.__init__(self, name, role, daemon=daemon, parent=parent, dependencies=dependencies)
        if len(kwargs) > 0:
