...
def open_files_and_stitch(infns: list, registration_method: int = REGISTER_IDENTITY, weaving_method: int = WEAVER_MEAN) -> list:
    """
    Stitches a set of tiles. If files contains multiple streams, each stream is assumed to be acquired
    at the same position, and they are stitched independently, so that the final result contains one
    stitched image for each stream.
    ...
    for i, tiles in enumerate(zip(*das_registered)):  # Iterate on the streams, with all the tiles in a tuple
