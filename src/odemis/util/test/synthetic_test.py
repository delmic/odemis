...
class TestParabolicMirrorRayTracer(unittest.TestCase):
    ...
    def test_simulate_caching_behavior(self):
        """
        Test that calling simulate() multiple times with the same position returns the cached image.
        """
        # Call simulate once to generate and cache an image for a specific position
        pos_a = {"x": 10e-6, "y": 10e-6, "z": -10e-6}
        img1 = self.tracer.simulate(pos_a)

        # Call simulate again with the exact same position
        img2 = self.tracer.simulate(pos_a)

        # The two images should be identical because the second call should be cached
        numpy.testing.assert_array_equal(
            img1,
            img2,
            "Subsequent calls with the same position should return the cached image.",
        )

        # Verify the ID of the returned array is different, confirming it's a copy
        self.assertNotEqual(
            id(self.tracer._last_img),
            id(img2),
            "Simulate should return a copy of the cached image, not the original.",
        )
    ...
    def test_simulate_handles_raytracing_failure_gracefully(self):
        ...
        with self.assertLogs(level="WARNING") as log:
            ...
            self.assertIn("Ray tracing failed. Using last image.", log.output[0])
