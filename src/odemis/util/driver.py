        ...
        for session in sessions:
            ...
            if len(parts) >= 3:
                ...
                session_info = subprocess.check_output(
                    ["loginctl", "show-session", session_id, "-p", "Type", "-p", "Active", "--value"],
                    text=True
                ).strip()
                stype, active = session_info.splitlines()

                if stype in ("wayland", "x11") and active == "yes":
                    gui_users.add(user)
...
def notify_to_user(target_user: str, title: str, message: str,
                   app: Optional[str] = None, icon: Optional[str] = None
                   ) -> None:
    """
