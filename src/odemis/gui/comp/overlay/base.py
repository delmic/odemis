...
class EdgeBoundingBox:
    ...
    t: float  # top, ymin
    b: float  # bottom, ymax
    index: Optional[int] = None  # Index of the edge, default None for a non-vertex edge
...
class Vec(tuple):
    ...
    def rotate(self, angle: float, point: Tuple[float, float]):
        """
        Rotate the vector by an angle around a point.

        :param angle: The angle by which the vector needs to be rotated in radians.
        :param point: The point (x, y) about which the vector needs to rotated.

        """
        dx = self.x - point[0]
        dy = self.y - point[1]
        x_rotated = point[0] + dx * math.cos(angle) - dy * math.sin(angle)
        y_rotated = point[1] + dx * math.sin(angle) + dy * math.cos(angle)
...
class LineEditingMixin(ClickMixin, DragMixin):
    ...
    def _calc_center(self):
        """Calculate the center of selection."""
        n = len(self.v_points)
        sum_x = sum(p.x for p in self.v_points)
        sum_y = sum(p.y for p in self.v_points)
        centroid_x = sum_x / n
        centroid_y = sum_y / n
