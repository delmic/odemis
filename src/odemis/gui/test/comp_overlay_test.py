...
class OverlayTestCase(test.GuiTestCase):
    ...
    def test_cryo_feature_overlay(self):
        ...
        def get_metadata():
            """ Get mocked values for simulated stage. """
            return {model.MD_FAV_FM_POS_ACTIVE: {"rx": 0.12213888553625313, "rz": 5.06145},
        ...
        stage.getMetadata = Mock(return_value=get_metadata())
        ...
        def getComponent(name="", role="stage-bare"):
            """
            Mock function to return a component based on its name and role.
            :return: fake component based on name and role
            """
            if role == "stage-bare":
                return stage
            elif role == "focus":
        ...
        def get_scan_rotation():
            """
            Mock function to return a scan rotation value without running the backend.
            :return: float scan rotation value
            """
            return 0.0
        ...
        with patch('odemis.gui.test.comp_overlay_test.getMicroscope', return_value=FakeMicroscope()), \
                ...
                patch('odemis.acq.move.MeteorPostureManager._get_scan_rotation', return_value=get_scan_rotation()):
