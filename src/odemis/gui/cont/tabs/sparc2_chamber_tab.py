...
class ChamberTab(Tab):
    ...
    def _update_progress_bar(self, pos):
        ...
        """
        # Mirror is not yet referenced, so its absolute position has no meaning.
        # Show the gauge at the midpoint to indicate an indeterminate/unknown position,
        # but only when the pulse animation isn't already running (during referencing).
        mirror = self.tab_data_model.main.mirror
        if not all(mirror.referenced.value.values()):
            if not self._pulse_timer.IsRunning():
                # Reset first, wx.Gauge won't repaint unless the value changes
                self.panel.gauge_move.Value = 0
                self.panel.gauge_move.Value = 50
            return
    ...
    def _on_switch_btn(self, evt):
        ...
        moves = []
        # If reference_axes_together is True, then the mirror axes can be referenced together and the
        # driver will take care of the referencing procedure. If False, reference the mirror
        # axes one-by-one.
        reference_axes_together = mirror_md.get(model.MD_CALIB, {}).get("reference_axes_together", False)
        if mstate == MIRROR_PARKED:
            ...
            for a in axes_order:
                ...
                if not reference_axes_together:
