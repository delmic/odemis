...
from odemis.acq.stream import LiveStream, StaticStream, SEMStream, FIBStream, OpticalStream, FluoStream
...
class FibsemTab(Tab):
    ...
    #         self._stage = self.tab_data_model.main.stage
        ...
        for vaname in get_local_vas(main_data.ebeam, main_data.hw_settings_config):
            if vaname in ("resolution", "dwellTime", "scale"):
                ...
                hwemtvas.add(vaname)
        sem_stream = SEMStream("SEM", electron_det, electron_det.data, electron_beam,     
                hwemtvas=hwemtvas,
                hwdetvas=None,
                emtvas=emtvas,
                detvas=get_local_vas(electron_det, main_data.hw_settings_config),
                blanker=None)
        
        emtvas = set()
        hwemtvas = set()
        for vaname in get_local_vas(main_data.ion_beam, main_data.hw_settings_config):
            if vaname in ("resolution", "dwellTime", "scale"):
                emtvas.add(vaname)
            else:
                hwemtvas.add(vaname)
        fib_stream = FIBStream("FIB", ion_det, ion_det.data, ion_beam,
                hwemtvas=hwemtvas,
                hwdetvas=None,
                emtvas=emtvas,
                detvas=get_local_vas(ion_det, main_data.hw_settings_config))
    ...
    #     #     )
        ...
        fib_stream_cont = self._streambar_controller.addStream(fib_stream, add_to_view=True)
