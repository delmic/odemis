...
class Sparc2AlignTab(Tab):
    ...
    def __init__(self, name, button, panel, main_frame, main_data):
        ...
        vpv = collections.OrderedDict((
            ...
            (self.panel.vp_align_light,
                {
                    "name": "Light alignment with CL Spot",
                    "stream_classes": (
            ...
            (self.panel.vp_align_light_ar,
                {
                    "name": "Light alignment with CL AR pattern",
                    "stream_classes": (
                        acqstream.CameraStream,
                        acqstream.Static2DUpdatableStream,
                    ),
        ...
        if ccd_focuser:  # TODO: handle fib_focuser as well
            ...
            for i, focus_stream in enumerate(self._focus_streams):
                # Add the stream to the stream bar controller
                # so that it's displayed with the default 0.3 merge ratio
                self._stream_controller.addStream(focus_stream, visible=False, add_to_view=False)
                # For the light in modes, we need the focus streams for the alignment process. They use a different view
                # than the default one, so instantiate them explicitly. Just add the first focus stream available for
                # now.
                if i == 0:
                    self._stream_controller.addStream(
                        focus_stream, visible=False, add_to_view=self.panel.vp_align_light.view
                    )
                    self._stream_controller.addStream(
                        focus_stream, visible=False, add_to_view=self.panel.vp_align_light_ar.view
                    )
    ...
    def _layoutModeButtons(self):
        ...
        # Update button sizes when content changed. When changing text after initial rendering, the size property
        # reflects the old text. We need a Layout() call to get the button sizes for the updated text. In the end
        # everything is positioned and we don't necessarily need another Layout() call.
        gb_sizer.Layout()
        # In the case of one button, don't show the button at all
        if len(btns) == 1:
        ...
        # In the case of two or four buttons, put them in two columns
            ...
            width = 2
        # In all other cases, try to see if they fit on three columns to preserve vertical space.
        else:
