...
class LocalizationTab(Tab):
    ...
    def __init__(self, name, button, panel, main_frame, main_data):
        ...
        # on the METEOR v2. To avoid this, we interpolate the content. The only drawback of
        # interpolation is that it lowers the frame rate, which can be annoying when manually
        # focusing, so we don't activate it by default.
