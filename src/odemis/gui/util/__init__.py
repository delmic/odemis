...
@call_in_wx_main
def enable_tab_on_stage_position(button, pos, target):
    """
    ...
    :param target: (int) target position label (IMAGING, SEM_IMAGING or FM_IMAGING)
    """
    if getCurrentPositionLabel(pos) in target:
        button.Enable(True)
    else:
        button.Enable(False)