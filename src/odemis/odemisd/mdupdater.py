        ...
        def updateOutWLRange(pos, fl=filter, comp_affected=comp_affected):
            ...
            if apply_transform:
                try:
                    ...
                    assert type(metadata_cor) == dict
                    assert all(isinstance(key, str) for key in metadata_cor)
                    comp_affected.updateMetadata(metadata_cor)
                except (AssertionError, KeyError) as exp:
                    logging.error(
                        "Chromatic correction metadata was not updated due to %s. Check if the CHROMATIC_COR is a "
                        "dictionary with filter band positions as keys and correction metadata dictionary as "
                        "values. For e.g. correction metadata dictionary for a given band position should "
                        "have the following format: {\"Pixel size cor\": [1, 1],\"Centre position cor\": [0, 0]"
                        " ,\"Rotation cor\": 0 ,\"Shear cor\": 1}", exp)
