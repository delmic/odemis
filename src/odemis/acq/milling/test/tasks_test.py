...
class MillingTaskTestCase(unittest.TestCase):
    ...
    def test_milling_settings2(self):
        ...
        json_data = milling_settings.to_dict()
        ...
        milling_settings_from_json = MillingSettings.from_dict(json_data)
    ...
    def test_milling_task_settings(self):
        ...
        json_data = milling_task_settings.to_dict()
        self.assertEqual(json_data["name"], "Milling Task")
        self.assertEqual(json_data["milling"], milling_settings.to_dict())
        self.assertEqual(json_data["patterns"][0], trench_pattern.to_dict())

        milling_task_settings_from_json = MillingTaskSettings.from_dict(json_data)
    ...
    def test_save_load_task_settings(self):
        ...
        trench_task_settings = MillingTaskSettings(milling_settings, [trench_pattern])

        milling_settings = MillingSettings(100e-9, 30e3, 400e-6, "Serial", "ion")
        microexpansion_pattern = MicroexpansionPatternParameters(1e-6, 10e-6, 100e-9, 10e-6, (0, 0))
        microexpansion_task_settings = MillingTaskSettings(milling_settings, [microexpansion_pattern])
