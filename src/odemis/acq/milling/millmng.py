...
class TFSMillingTaskManager:
    ...
    def run_milling(self, settings: MillingTaskSettings):
        ...
        try:
            ...
            self._future.set_progress(total_time=self._future.elapsed_time + estimated_time)
...
def run_milling_tasks(tasks: List[MillingTaskSettings], fib_stream: FIBStream, filename: str = None) -> Future:
    ...
    future.set_progress(total_time=future.elapsed_time + 10 * len(tasks))
...
def run_automated_milling(features: List[CryoFeature],
    ...
    future.set_progress(total_time=future.elapsed_time + total_duration) # TODO: get proper time estimate from fibsemOS
