...
class SEMTemporalSpectrumMDStream(SEMCCDMDStream):
    ...
    def _runAcquisition(self, future) -> Tuple[List[model.DataArray], Optional[Exception]]:
        ...
        try:
            ...
            intensity_limit_s = ccd_md.get(model.MD_CALIB, {}).get("intensity_limit", 40000)
            self._intensity_limit = min(intensity_limit_s * exp, max_det_value)
            logging.debug("Streak CCD intensity threshold set to %s", self._intensity_limit)
    ...
    def _check_light_intensity(self, raw_data):
        ...
        num_high_px = numpy.sum(raw_data > self._intensity_limit)
        if num_high_px > STREAK_CCD_INTENSITY_MAX_PX_COUNT:
            ...
            # exception anyway.
            self._sccd._suspend()
            raise ValueError(f"Light intensity too high ({num_high_px} px > {self._intensity_limit}), stopping acquisition. "
