...
class MultipleDetectorStream(Stream, metaclass=ABCMeta):
    ...
    def _getCenterPositionPhys(self) -> Tuple[float, float]:
        """
        Compute the center position of the RoA in physical coordinates (ie, corresponding to the
        stage coordinates).
        :return: position (x, y) of the center of the RoA in absolute physical coordinates (m, m)
        """
        roi = self.roi.value
        center_roi = ((roi[0] + roi[2]) / 2, (roi[1] + roi[3]) / 2)

        shape = self._emitter.shape
        # convert into SEM translation coordinates: distance in px from center of FoV (0.5)
...
class SEMCCDMDStream(MultipleDetectorStream):
    ...
    def _select_polarization(self, pol_pos: Optional[str]) -> float:
        """
        Configure the optical path for the given polarisation position.
        :param pol_pos: (A value of POL_POSITIONS or None) The polarization position for the
        "pol-analyzer" component. If None, then no movement is done (ie, no polarimetry analysis is performed).
    ...
    def _assembleFinalData(self, n, data):
        ...
        if self._analyzer and data:
            # If more than data, it means it was acquired with multiple polarizations => keep them
            # separated, and add the polarization name to the description.
            # If only one data, check if polarization mode metadata is present, in which case also
            # add polarization information to description, to make it consistent.
            if len(data) > 1 or data[0].metadata.get(model.MD_POL_MODE, MD_POL_NONE) != MD_POL_NONE:
                for d in data:
                    if model.MD_POL_MODE in d.metadata:
                        d.metadata[model.MD_DESCRIPTION] += " " + d.metadata[model.MD_POL_MODE]
                    else:
                        logging.warning("No polarization mode metadata in CCD data: %s", d.metadata)
...
class SEMMDStream(MultipleDetectorStream):
    ...
    def _adjustHardwareSettings(self):
        ...
        # TODO: check that no fuzzing is requested (as it's not supported and not useful).
    ...
    def _runAcquisitionVector(self, future) -> Tuple[List[model.DataArray], Optional[Exception]]:
        ...
        try:
            ...
            while spots_sum < tot_num:
                ...
                # Current pixel index: compute it from the number of spots scanned so far, based on
                # the full repetition shape (and knowning that X iterates first)
                px_idx = (spots_sum // rep[0], spots_sum % rep[0])  # Y, X
                acq_rect = (px_idx[1], px_idx[0], px_idx[1] + n_x - 1, px_idx[0] + n_y - 1)
                self._update_live_area(acq_rect, in_progress=True)
...
class ScannedFluoMDStream(MultipleDetectorStream):
    ...
    def _runAcquisition(self, future) -> Tuple[List[model.DataArray], Optional[Exception]]:
        ...
        try:
            ...
            # TODO: immediately remove the synchronisation? It's not needed after the start.
...
class SEMCCDAcquirer(abc.ABC):
    ...
    def complete_spatial_acquisition(self, pol_idx) -> List[Optional[model.DataArray]]:
        ...
        :param pol_idx: polarization index for which this spatial acquisition is run
        :return: The DataArrays newly acquired, for the whole spatial acquisition, per stream.
        ...
        while True:  # Done only once normally, excepted in case of failures
            ...
            if timedout or dur < self.snapshot_time * 0.95:
                if timedout:
                    ...
                    logging.warning(
                        "Acquisition of repetition stream for pixel %s timed out after %g s. "
...
class SEMCCDAcquirerVector(SEMCCDAcquirerRectangle):
    ...
    def prepare_hardware(self, max_snapshot_duration: Optional[float] = None) -> None:
        # Note: the parent prepare_hardware() does a lot too much, as it's not necessary to configure,
        # the scanner for normal scanning, but it works fine.
...
class SEMCCDAcquirerScanStage(SEMCCDAcquirerRectangle):
    ...
    * sample stage: use the sample stage to scan. Slower & less accurate, but cheaper.
    ...
    def _move_scanner(self, px_idx: Tuple[int, int]) -> None:
        ...
        # Note: we don't shift the e-beam to compensate for the drift, because one of the reason
        # for using the scan stage is to be able to use other emitter (eg, laser), which cannot move.
        clipped_spos = {"x": px_pos_scan[0], "y": px_pos_scan[1]}
...
class SEMCCDAcquirerHwSync(SEMCCDAcquirer):
    ...
    def prepare_hardware(self, max_snapshot_duration: Optional[float] = None) -> None:
        ...
        # done via the SettingsStream (except for the exposureTime if integrationTime is used, as seen above).
