...
class SEMCCDMDStream(MultipleDetectorStream):
    ...
    def _update_live_area(self, px_idx, tile_size):
    ...
    def _run_acquisition_ccd(self, future: model.ProgressiveFuture, acquirer: "SEMCCDAcquirer"
        ...
        try:
            ...
            shortest_leech_period = self._get_min_leech_period()  # TODO: test
            ...
            for pol_idx, pol_pos in enumerate(pos_polarizations):
                ...
                for px_idx in numpy.ndindex(*rep[::-1]):  # last dim (X) iterates first
                    ...
                    self._update_live_area(px_idx, acquirer.tile_size)
            ...
            self._live_data = [[] for _ in self._streams]  # FIXME: can be put an empty list?
            self._streams[0].raw = []
        ...
        if timedout:
            ...
            while time.time() < endt:
                ...
                if not timedout:
                    break
        if not timedout:
            logging.debug("Got synchronized acquisition from detector %d." % self._mdstream._ccd_idx)
