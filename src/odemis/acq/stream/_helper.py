...
class ARSettingsStream(PolarizedCCDSettingsStream):
    ...
    """
    def __init__(self, name, detector, dataflow, emitter, analyzer, **kwargs):

        if "acq_type" not in kwargs:
            kwargs["acq_type"] = model.MD_AT_AR

        super(ARSettingsStream, self).__init__(name, detector, dataflow, emitter, analyzer, **kwargs)
...
class AngularSpectrumSettingsStream(PolarizedCCDSettingsStream):
    ...
    """
    def __init__(self, name, detector, dataflow, emitter, spectrometer, spectrograph, analyzer, **kwargs):

        if "acq_type" not in kwargs:
            kwargs["acq_type"] = model.MD_AT_EK

        super(AngularSpectrumSettingsStream, self).__init__(name, detector, dataflow, emitter, analyzer,
                                                            **kwargs)  # init of PolarizedCCDSettingsStream
    ...
    def _onNewData(self, dataflow, data):
        ...
        data.metadata[model.MD_DIMS] = "CAZYX"
        data.metadata[model.MD_THETA_LIST] = angleres.ExtractThetaList(data)

        self._calcWLList()
        data.metadata[model.MD_WL_LIST] = self.wllist
        logging.debug("WL_LIST = %s (from CCD = %s)", self.wllist, self.spectrograph)
    ...
    def _onBinning(self, _=None):
        ...
        """
        try:
            self._detector.binning.value = (self.spectrum_binning.value, self.angular_binning.value)
        except Exception:
            logging.warning("Different values of horizontal and vertical binning than (%s, %s) are expected "
                            "on the detector", self.spectrum_binning.value, self.angular_binning.value)

        try:
            cam_xres = self._detector.shape[0] // self._detector.binning.value[0]
            cam_yres = self._detector.shape[1] // self._detector.binning.value[1]
    ...
    def _calcWLList(self):
        """
        Calculates wavelength list and checks whether it should be inverted
        """
        try:
            cam_xres = self._detector.shape[0] // self._detector.binning.value[0]
            self.wllist = self.spectrograph.getPixelToWavelength(cam_xres,
                                                        self._detector.pixelSize.value[0] *
                                                        self._detector.binning.value[0])
        except Exception:
            logging.exception("Failed to calculate wavelength list")
