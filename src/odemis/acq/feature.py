...
class CryoFeatureAcquisitionTask(object):
    ...
    def _acquire_feature(self, site: CryoFeature):
        ...
        if not data:
            ...
            logging.info(
                "The acquisition for stream %s for feature %s is done",
                self.streams,
                site.name,
            )
    ...
    def _move_to_site(self, site: CryoFeature):
        ...
        stage_pos = {
            "x": site.pos.value[0],
            "y": site.pos.value[1],
            # "z": site.pos.value[2] # TODO: fix not available...
        }
    ...
    def estimate_acquisition_time(self) -> float:
        ...
        n_active_features = sum(
            1 for f in self.features if f.status.value == FEATURE_ACTIVE
        )
        logging.debug(
            f"Estimated total acquisition time for {n_active_features} features: {acq_time * n_active_features} s"
        )

        return acq_time * n_active_features

    def estimate_movement_time(self) -> float:
        """Estimate the movement time for the acquisition task."""
        # TODO: implement
        expected_stage_time = 0.1
    ...
    def run(self):
        ...
        try:
            ...
            for feature in self.features:
                ...
                # d = feature.streams.value[-1].raw[0]
                # n_zlevels = 1
                # if self.zlevels:
                #     n_zlevels = len(list(self.zlevels.values())[0])
                # plt.title(f"{feature.name.value}: z-levels: {n_zlevels}")
    ...
    def _export_data(self, feature, data):
        """
        Called to export the acquired data.
        data (DataArray): the returned data/images from the future
        """

        def _add_feature_info_filename(feature, filename: str) -> str:
            """
        ...
        filename = _add_feature_info_filename(feature, self.filename)
...
def acquire_at_features(
    ...
    :param stage: (Stage) The stage to move to the features.
    :param focus: (Focus) The focus to move to the features.
    :param streams: (List[Stream]) The streams to acquire data from.
    :param filename: (str) The filename to save the acquired data to.
    :param zlevels: (List[float]) The z-levels to acquire data at.
    :param settings_obs: (SettingsObserver) The settings observer to use for the acquisition.
    :return: (Dict[CryoFeature, List[Stream]]) The acquired data for each feature.
    """
