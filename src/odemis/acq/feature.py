...
def feature_decoder(feature_raw: Dict) -> CryoFeature:
    ...
    :param feature_raw: a serialized version of a feature that is to be decoded into an Odemis feature.
    :return: the deserialzed version of the feature
    """
    ...
    is_collectible = feature_raw.get('is_collectible', False)
...
def collect_feature_data(
    ...
    """
    Collect anonymized data for a feature and submit it to the data collector. It skips immediately if feature.is_collectible
    ...
    After collection feature.is_collectible is set to False to prevent re-collection.

    :param feature: The feature for which the data is collected. Must have feature.is_collectible == True.
    ...
    try:
        dc = datacollector.get_data_collector()
        if not dc.get_consent():
            return
    except Exception:
    ...
    try:
        ...
        def _get_raw(stream: Stream) -> Optional[model.DataArray]:
            """Return the first raw image for a static stream, if available."""
            try:
                raw = stream.raw
            except Exception:
                return None
    ...
    def run(self) -> Tuple[List[model.DataArray], Exception]:
        ...
        try:
            with self._future._task_lock:
                ...
                logging.debug(f"Current Posture: {current_posture}")

                if current_posture != Posture.FM_IMAGING:
                    raise ValueError(f"Currently only supported when at {Posture.FM_IMAGING} posture")
                    # TODO: enable this when the cryo switch is fixed
                    logging.debug(
                        f"Moving to {Posture.FM_IMAGING} from {current_posture}"
                    )
                    self._future.running_subf = self.pm.switch_posture(Posture.FM_IMAGING)
