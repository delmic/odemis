...
class MeteorTFS1PostureManager(MeteorPostureManager):
    def __init__(self, microscope):
        ...
        # TODO: only fill this in if not present in metadata already
        convert_stage = model.getComponent(name="Linked YZ")
        pre_tilt = stage_md.get(model.MD_SAMPLE_PRE_TILT,
            -convert_stage.getMetadata()[model.MD_ROTATION_COR])
        # TODO: assert convert negative pre-tilt to positive

        # default override for translation correction
        trans_cor = stage_md.get(model.MD_POS_TRANS_COR, MD_TRANSLATION_COR)

        # milling position
        mill_pos_active = copy.deepcopy(stage_md[model.MD_FAV_SEM_POS_ACTIVE])
        # sem position
        sem_pos_active = copy.deepcopy(stage_md[model.MD_FAV_SEM_POS_ACTIVE])
        sem_pos_active["rx"] = pre_tilt
        # fib position
        fib_pos_active = copy.deepcopy(stage_md[model.MD_FAV_SEM_POS_ACTIVE])
        fib_pos_active["rx"] = math.radians(52) - (pre_tilt)
        fib_pos_active["rz"] = fib_pos_active["rz"] + math.radians(180)
