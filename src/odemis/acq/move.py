...
class MeteorPostureManager(MicroscopePostureManager):
    ...
    def getCurrentPostureLabel(self, pos: Dict[str, float] = None) -> int:
        ...
        mill_fav_pos = stage_md.get(model.MD_FAV_MILL_POS_ACTIVE, {})
        fib_fav_pos = stage_md.get(model.MD_FAV_FIB_POS_ACTIVE, {})
        if pos is None:
...
class MeteorTFS1PostureManager(MeteorPostureManager):
    def __init__(self, microscope):
        ...
        # QUERY: remove the below if we choose to require this in the yaml file,
        # this is only so we don't need to edit yaml
        # Add to required keys then

        # dont re-init over existing metadata
        if {
            model.MD_FAV_FIB_POS_ACTIVE,
            model.MD_FAV_MILL_POS_ACTIVE,
            model.MD_FAV_SEM_POS_ACTIVE,
            model.MD_FAV_FM_POS_ACTIVE,
            model.MD_SAMPLE_PRE_TILT,
        }.issubset(stage_md):
            return
        ...
        if model.MD_SAMPLE_PRE_TILT not in stage_md:
            convert_stage = model.getComponent(name="Linked YZ")
            pre_tilt_correction = abs(
                -convert_stage.getMetadata()[model.MD_ROTATION_COR]
            )
        pre_tilt = stage_md.get(model.MD_SAMPLE_PRE_TILT, pre_tilt_correction)

        # default override for translation correction
        trans_cor = stage_md.get(model.MD_POS_TRANS_COR, MD_TRANSLATION_COR)

        # milling position
        mill_pos_active = copy.deepcopy(
            stage_md.get(
                model.MD_FAV_MILL_POS_ACTIVE, stage_md[model.MD_FAV_SEM_POS_ACTIVE]
            )
        )
        # sem position
        sem_pos_active = copy.deepcopy(stage_md[model.MD_FAV_SEM_POS_ACTIVE])
        sem_pos_active["rx"] = pre_tilt

        # fib position
        fib_pos_active = stage_md.get(
            model.MD_FAV_FIB_POS_ACTIVE, stage_md[model.MD_FAV_SEM_POS_ACTIVE]
        )
        if model.MD_FAV_FIB_POS_ACTIVE not in stage_md:
            fib_pos_active["rx"] = (
                math.radians(52) - pre_tilt
            )  # 52 degrees is the FIB column tilt (TFS)
            fib_pos_active["rz"] = sem_pos_active["rz"] + math.radians(
                180
            )  # 180 degrees rotation from SEM to FIB

        # update metadata
        self.stage.updateMetadata(
            {
                model.MD_FAV_SEM_POS_ACTIVE: sem_pos_active,
                model.MD_FAV_MILL_POS_ACTIVE: mill_pos_active,
                model.MD_FAV_FIB_POS_ACTIVE: fib_pos_active,
                model.MD_SAMPLE_PRE_TILT: pre_tilt,
                model.MD_POS_TRANS_COR: trans_cor,
            }
        )
