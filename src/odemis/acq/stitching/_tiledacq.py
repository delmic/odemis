...
class TiledAcquisitionTask(object):
    ...
    def _sortDAs(self, das, ss):
        ...
        def leader_quality(da: model.DataArray) -> float:
            """
            Estimate how good a data is for computing registration between tiles.
            It favors images with more pixels, and EM images over others (as they typically have
            more contrast and smaller pixel size).
            :return: (> 0) The bigger, the better (at "leading" the stitching)
            """
    ...
    def _acquireStreamTile(self, i, ix, iy, stream):
        ...
        self._future.set_progress(total_time=self._future.elapsed_time + self.estimateTime(self._number_of_tiles - i))
        ...
        try:
            ...
            if self._registrar is not None and self._weaver is not None:
                if not da_list or not da_list[0]:
                    ...
                    self._future.set_progress(total_time=self._future.elapsed_time + self.estimateTime(0))
...
def acquireTiledArea(streams, stage, area, overlap=0.2, settings_obs=None, log_path=None, zlevels=None,
    ...
    future.set_progress(total_time=future.elapsed_time + task.estimateTime())
...
def acquireOverview(streams, stage, areas, focus, detector, overlap=0.2, settings_obs=None, log_path=None, zlevels=None,
    ...
    future.set_progress(total_time=future.elapsed_time + task.estimate_time())
    # connect the future to the task and run in a thread
    executeAsyncTask(future, task.run)
...
class AcquireOverviewTask(object):
    ...
    def _pass_future_progress(self, future, elapsed_time: float, estimated_total_time: float) -> None:
        # update the main future with running sub future time estimate and time estimation of remaining tasks
        self._future.set_progress(total_time=estimated_total_time + sum(self.time_per_task))
    ...
    def run(self):
        ...
        da_rois = []
        try:
            self._future.set_progress(total_time=self._future.elapsed_time + self.estimate_time())
