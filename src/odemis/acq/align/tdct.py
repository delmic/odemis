...
# tmp: append the development path until we have an installation
...
def convert_das_to_numpy_stack(das: List[model.DataArray]) -> numpy.ndarray:
    ...
    for da in das:
        if isinstance(da, model.DataArrayShadow):
            da = da.getData()
        if da.ndim == 5: # model.DataArray is always 5D for meteor z-stacks
            da = da[0, 0, :, :, :]
        arr.append(da)
...
def run_tdct_correlation(fib_coords: numpy.ndarray,
    ...
    correlation_results = run_correlation(
        ...
        path=path,
        fib_image_filename=fib_image.metadata[model.MD_DESCRIPTION],
        fm_image_filename=fm_image.metadata[model.MD_DESCRIPTION],
    )
