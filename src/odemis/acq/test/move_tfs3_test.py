...
from odemis.acq.move import MeteorTFS3PostureManager, MicroscopePostureManager
...
class TestMeteorTFS3Move(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if self.pm.current_posture.value == Posture.UNKNOWN:
            ...
            f = self.pm.switch_posture(Posture.LOADING)
            f.result()
    ...
    def test_switching_movements(self):
        ...
        if model.MD_FAV_MILL_POS_ACTIVE in self.stage_md:
            f = self.pm.switch_posture(Posture.MILLING)
            f.result()
    ...
    def test_to_posture(self):
        ...
        f = self.stage_bare.moveAbs(self.stage_grid_centers[Posture.GRID_1.value])
        f.result()

        # Check that get_current_posture() with a given stage-bare position returns the expected posture
        pos = self.stage_bare.position.value
        self.assertEqual(self.pm.get_current_posture(pos), Posture.SEM_IMAGING)

        fm_pos = self.pm.to_posture(pos, Posture.FM_IMAGING)
        self.assertEqual(self.pm.get_current_posture(fm_pos), Posture.FM_IMAGING)

        if model.MD_FAV_MILL_POS_ACTIVE in self.stage_md:
            milling_pos = self.pm.to_posture(pos, Posture.MILLING)
            self.assertEqual(self.pm.get_current_posture(milling_pos), Posture.MILLING)
    ...
    def test_sample_stage_movement(self):
        ...
        for posture in [Posture.SEM_IMAGING, Posture.FM_IMAGING]:
            ...
            if self.pm.current_posture.value != posture:
                f = self.pm.switch_posture(posture)
                f.result()

            f = self.pm.switch_posture(Posture.GRID_1)
            f.result()
            ...
            for posture in self.pm.postures:
                ...
                if pos_sample_ref is None:
                    ...
                    if posture == Posture.FM_IMAGING:
                        ...
                        pos_sample_noz.pop("z")

                        pos_sample_ref_noz = pos_sample_ref.copy()
                        pos_sample_ref_noz.pop("z")
                        testing.assert_pos_almost_equal(pos_sample_noz, pos_sample_ref_noz, atol=1e-6)
                    else:
    ...
    def test_fixed_fm_z(self):
        ...
        """
        translational = self.stage_grid_centers[Posture.GRID_1.value]
