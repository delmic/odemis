...
#         md[model.MD_SW_VERSION] = val
    ...
    if fibsemos_md is not None:
        try:
            ...
            logging.debug(f"Failed to parse FIBSEM-OS metadata: {e}")
...
def _convert_fibsemos_to_odemis_metadata(metadata: str) -> dict:
    ...
    try:
        ...
        raise ValueError(f"Failed to parse FIBSEM-OS metadata json: {e}")
    except KeyError as e:
        raise ValueError(f"Invalid json structure for FIBSEM-OS metadata: {e}")
    ...
    try:
        ...
        logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_ACQ_DATE} - {type(e)}: {e}")

    try:
        system_info = omd["system"]["info"]
        md[model.MD_HW_NAME] = system_info["manufacturer"] + " " + system_info["model"] + "-" + system_info["serial_number"]
    except KeyError as e:
        logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_HW_NAME} -  {type(e)}: {e}")
    ...
    try:
        ...
        logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_SW_VERSION} - {type(e)}: {e}")

    try:
        md[model.MD_PIXEL_SIZE] = (float(omd["pixel_size"]["x"]),
                                   float(omd["pixel_size"]["y"]))
    except KeyError as e:
        logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_PIXEL_SIZE} - {type(e)}: {e}")
    ...
    try:
        ...
        try:
            ...
            logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_DWELL_TIME} - {e}")
        try:
            # MD_POS should match image dimensions, e.g. only XY for 2D, XYZ for 3D, etc.
            ...
            logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_POS} - {e}")
        ...
        try:
            ...
            logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_ACQ_TYPE} - {e}")
        ...
        try:
            ...
            logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_BEAM_SCAN_ROTATION} - {e}")

        try:
            md[model.MD_EBEAM_VOLTAGE] = float(beam_settings["voltage"])
        except KeyError as e:
            logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_EBEAM_VOLTAGE} - {e}")

        try:
            md[model.MD_EBEAM_CURRENT] = float(beam_settings["beam_current"])
        except KeyError as e:
            logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_EBEAM_CURRENT} - {e}")
        ...
        try:
            ...
            logging.warning(f"Failed to parse FIBSEM-OS metadata: {model.MD_DET_TYPE} - {e}")

    except Exception as e:
        logging.warning(f"Failed to parse FIBSEM-OS metadata: {e}", exc_info=True)
    ...
    if not md:
        raise ValueError("ImageDescription tag contained data, but no FIBSEM-OS metadata found... falling back to previous metadata...")
