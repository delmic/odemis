...
class AcquisitionDataTIFF(AcquisitionData):
    ...
    def __init__(self, filename):
        ...
        """
        try:
            # attempt to open images with new OME-TIFF format, validated by OME-XML
            data = open_data_ome_tiff(filename)
            thumbnails = []
        except Exception as e:
            logging.debug(f"Failed to open the file as OME 2016-06 TIFF, retrying with standard TIFF reader: {e}")
...
def reformat_ome_metadata(image_data: List[model.DataArray],
    ...
    for nc in range(size_c):
        ...
        channels.append(
            Channel(
                ...
                samples_per_pixel=1,
                color=Color(channel_md[nc]["user_tint"]),
        ))
        ...
        for nz in range(size_z):
            ...
            # add plane
            # TODO: the position_z is incorrect, it's currently the same for all planes...
            planes.append(
                Plane(
    ...
    image = Image(
        ...
        pixels=Pixels(
            id="Pixels:0",
            dimension_order=Pixels_DimensionOrder.XYZCT, # query: confirm dimension order
...
def open_data_ome_tiff(path: str) -> List[model.DataArray]:
    ...
    :return: (list) of DataArray objects with metadata
    """

    # note: this function is currently best effor loading.
    import json
    np = numpy
    import tifffile as tff
    ...
    # reformat the image data, assumption is T = 1 always therefore ndim == 4
    if arr.ndim == 4:
        # insert at pos 1
    ...
    image_data = []
    nc = ome.images[0].pixels.size_c
    nz = ome.images[0].pixels.size_z
    nt = ome.images[0].pixels.size_t
    for c, md in zip(range(nc), mds):
        ...
        # ZYX -> 5D
        # CTZYX -> 5D
        logging.info(f"shape: {d.shape}, dim: {md[model.MD_DIMS]}")
