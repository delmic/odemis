...
def _convert_jeol_to_odemis_metadata(tfile, jeol_md: dict[str, str]) -> dict[str, Any]:
    ...
    :param tfile: the opened TIFF file
    ...
    # There is (often) no metadata about pixel size. However, there is sometimes there is a
    # CM_FIELD_OF_VIEW, defined like "1.067mm 0.800mm", and sometimes defined just as "1.067 0.800mm".
    ...
    if "CM_FIELD_OF_VIEW" in jeol_md:
        try:
            ...
            if len(tokens) == 2:
                ...
                # fov_y = float(fov_y_str.replace("mm", "")) * 1e-3  # in m
    ...
    if "CM_ACCEL_VOLT" in jeol_md:
        try:
            ...
            logging.debug("Failed to parse JEOL beam voltage '%s': %s", jeol_md["CM_ACCEL_VOLT"], ex)
