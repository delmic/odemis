...
def export(filename, data, thumbnail=None, compressed=True, multiple_files=False, pyramid=False):
    ...
    if GLOBAL_OME_COMPAT_MODE:
        try:
            ...
            reformat_ome_metadata(image_data=data, new_fn=new_fn)
            logging.info(f"Reformatted OME metadata saved as {new_fn}")
        except Exception as e:
...
class AcquisitionDataTIFF(AcquisitionData):
    ...
    def __init__(self, filename):
        ...
        """
        try:
            if not GLOBAL_OME_COMPAT_MODE:
                raise ValueError("OME 2016 compatibility is disabled")
            data = open_data_ome_tiff(filename)
            thumbnails = []
        except Exception as e:
            logging.debug(f"Failed to open the file as OME 2016-06 TIFF: {e}")
            logging.debug("Retrying with standard TIFF reader")
...
def reformat_ome_metadata(image_data: List[model.DataArray] = None,
                          filename: str = None,
                          new_fn: str = None,
    ...
    if filename is not None:
        logging.info(f"Opening image: {filename}")
    ...
    d0 = image_data[0]
    if isinstance(d0, model.DataArrayShadow):
        d0 = d0.getData()
    if d0.ndim == 5:
        size_t = d0.shape[-4]
        size_z = d0.shape[-3]
    ...
    logging.info(f"Image Dimensions (CTZYX):  {data.shape}")

    # for debugging synthetic data (z-index gets brighter)
    if DEBUG:
        logging.warning("WARN: DEBUG is turned on, images will be manipulated.")
        if size_z > 1:
    ...
    roi = None
    if model.MD_ROTATION in d0.metadata or model.MD_SHEAR in d0.metadata:
        rot = d0.metadata.get(model.MD_ROTATION, 0)
        sinr, cosr = math.sin(rot), math.cos(rot)
        she = d0.metadata.get(model.MD_SHEAR, 0)
        m = [[cosr + sinr * she, sinr, 0],
             [-sinr + cosr * she, cosr, 0]]
        affine_transform = AffineTransform(
            a00=m[0][0], a01=m[0][1], a02=m[0][2],
            a10=m[0][0], a11=m[1][1], a12=m[1][2]
        )

        # create a Shape (Rectangle) same size as image with the transformation
        shape = Rectangle(
            id="Shape:0",
            x=0,
            y=0,
        ...
        # create an ROI to represent the whole image transformation
        roi = ROI(id="ROI:0", description="Image Transformation", union=[shape])
    ...
    for nc in range(size_c):
        ...
        for nz in range(size_z):
            ...
            tiff_data_blocks.append(
                TiffData(plane_count=1, ifd=ifd, first_z=nz, first_c=nc, first_t=0)
            )

            # add plane
            planes.append(
                Plane(
                    the_c=nc, the_t=0, the_z=nz, exposure_time=exp_time,
    ...
    image = Image(
        ...
        pixels=Pixels(
            id="Pixels:0",
            dimension_order=Pixels_DimensionOrder.XYZCT, # QUERY: is this correct? should be XYZTC?
    ...
    # add the image transformation as a ROI
    if roi:
        ome.rois.append(roi)

    # store extra settings as structured annotations (must be str)
    map_annotations = []
    for i, c in enumerate(range(size_c)):
        md = image_data[c].metadata
        map_annotation_md = MapAnnotation(id=f"Annotation:{c}", value={"metadata": json.dumps(md)})
        map_annotations.append(map_annotation_md)

    sa = StructuredAnnotations(map_annotations=map_annotations)
    ome.structured_annotations = sa

    # store plane specific metadata (e.g. correction factors) as structured annotations
    ...
    logging.info(f"Image saved to {new_fn} with OME-XML metadata.")
...
def open_data_ome_tiff(path: str) -> List[model.DataArray]:
    ...
    from ome_types import from_tiff, from_xml, to_dict

    with tff.TiffReader (path) as tif:
    ...
    for mp_anno in mp_annos:
        ...
        md_dict = to_dict(mp_anno.value)
        md = json.loads(md_dict["metadata"])
        mds.append(md)

    print(arr.shape)
    print(mds[0][model.MD_DIMS])

    # reformat the image data
    # TODO: this does not work for T data, only C,Z,Y,X.
    # assumption is T = 1 always
    if arr.ndim == 4:
        # insert at pos 1
    ...
    image_data = []
    nc = arr.shape[0]
    for c, md in zip(range(nc), mds):
        # if ndim == 4, expand to 5D
        d = arr[c, :, :, :, :]
        if d.ndim == 4:
            d = np.expand_dims(d, axis=1)

        # print(f"shape after expand: {d.shape}")

        # YX -> 2D
        # ZYX -> 5D
        # CTZYX -> 5D
        print(d.shape, md[model.MD_DIMS])

        if d.ndim == 5: #md[model.MD_DIMS] == "YX":
            if (d.shape[0] == 1 and d.shape[1] == 1 and d.shape[2] == 1):
                d = d[0, 0, 0, :, :]
                # print("YX")
                # print(d.shape)
        da = model.DataArray(d, md)
        image_data.append(da)
