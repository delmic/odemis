...
#    @skip("simple")
    ...
    def testFindImageGroupsAcquiredMultiChannelZStack(self):
        ...
        # check images match acqusition format
        self.assertEqual(len(ldata), nb_c)              # Check that the number of images is correct (nb_c)
        self.assertEqual(ldata[0].shape, size[::-1])    # Check that the shape of the images is correct (size[::-1])
        self.assertEqual(ldata[0].ndim, 3)              # Check that the images are 3D
    ...
    def test_convert_zeiss_to_odemis_metadata(self):

        # open example image
        ZEISS_FILENAME = os.path.join(os.path.dirname(__file__), "zeiss_example_sem.tif")
        data = tiff.open_data(ZEISS_FILENAME)

        # assert data
        self.assertIsInstance(data, tiff.AcquisitionDataTIFF)
        self.assertEqual(len(data.content), 1)
        self.assertEqual(data.content[0].shape, (768, 1024))
        self.assertEqual(data.content[0].dtype, numpy.uint8)
        ...
        md = {
            model.MD_ACQ_DATE: md_acq_date,
            model.MD_HW_NAME: "Crossbeam 550-8404010334",
            model.MD_SW_VERSION: "V07.03.00.00 : 07-Feb-23",
            model.MD_PIXEL_SIZE: (5.933e-07, 5.933e-07),
            model.MD_DWELL_TIME: 50e-9,
            model.MD_POS: (0.048509000000000004, 0.0486715),
            model.MD_EBEAM_VOLTAGE: 5000.0,
            model.MD_EBEAM_CURRENT: 2e-10,
            model.MD_EBEAM_SPOT_DIAM: 1.0,
            model.MD_ACQ_TYPE: model.MD_AT_EM,
            model.MD_DET_TYPE: model.MD_DT_NORMAL,
            model.MD_DESCRIPTION: os.path.basename(ZEISS_FILENAME).replace(".tif", ""),
        }
    ...
    def test_convert_tescan_to_odemis_metadata(self):
        ...
        md = {
                ...
                # model.MD_SW_VERSION: "TESCAN Essence Version 1.2.2.0, build 5793", # missing key
                model.MD_PIXEL_SIZE: (2.5012e-06, 2.5012e-06),
                model.MD_DWELL_TIME: 3.2e-06,
                model.MD_POS: (0.0051572, 0.0060938),
                model.MD_EBEAM_VOLTAGE: 10000.0,
                model.MD_EBEAM_CURRENT:  2.64366291318e-10,
    ...
    def test_convert_jeol_to_odemis_metadata(self):
        ...
        # assert metadata
        md = {
            # To deal with the fact that the date is store in the local timezone, while MD_ACQ_DATE
            ...
            model.MD_PIXEL_SIZE: (1.984375e-6, 1.984375e-6),
