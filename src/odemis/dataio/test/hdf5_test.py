...
# TODO: test compatibility with Hyperspy for loading spectra (exported by Odemis)
    ...
    def testReadAndSaveMDARPOL(self):
        """
        Checks that we can read back the metadata of an Angular Resolved image with an polarization analyzer
        """
        metadata = [{model.MD_SW_VERSION: "1.0-test",
                     model.MD_HW_NAME: "fake hw",
                     model.MD_DESCRIPTION: "sem survey",
                    ...
                    # same AR image MD but different polarization position (MD_ARPOL_POLARIZATION)
                    {model.MD_SW_VERSION: "1.0-test",
                     model.MD_HW_NAME: "fake ccd",
                     model.MD_DESCRIPTION: "POL2",
                     model.MD_ACQ_DATE: time.time(),
                     model.MD_BPP: 12,
                     model.MD_BINNING: (1, 1),  # px, px
                     model.MD_SENSOR_PIXEL_SIZE: (13e-6, 13e-6),  # m/px
                     ...
                     model.MD_AR_PARABOLA_F: 2e-3,
                     model.MD_LENS_MAG: 60,  # ratio
                     model.MD_ARPOL_POLARIZATION: "horizontal",
                     model.MD_ARPOL_POS_QWP: 0.0,  # rad
                     model.MD_ARPOL_POS_LINPOL: 0.0,  # rad
                     },
                    ]

        # TODO create test case with 6 images each with a different polarization??
        # TODO I created two different MD with different pol pos but same pos of ebeam. enough? or still all six
