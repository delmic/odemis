...
class SEM(model.HwComponent):
    ...
    def __init__(self, name, role, children, host, daemon=None, **kwargs):
        ...
        if "scanner" in children:
            ...
            for name, ckwargs in children.items():
                if name.startswith("detector"): # Matches only SEM detectors (detector0, detector1, etc.)
    ...
    def _single_acquisition(self, detector):
        ...
        device_type = scanner._device_handler.device_type
        with self._acquisition_init_lock:
            ...
            logging.debug(f"Acquiring {DEVICE_LABELS[device_type]} image of {res} with dwell time {dt} ns")
        ...
        with self._acq_progress_lock:
            try:
                ...
                dtype = numpy.uint8 if detector.bpp.value == 8 else "<u2"
                img = numpy.frombuffer(img, dtype=dtype)
                logging.debug(f"Received {DEVICE_LABELS[device_type]} image of length {len(img)}")
            except OSError:
...
class Scanner(model.Emitter):
    ...
    def _setBeamPreset(self, preset):
        self._device_handler.ScStopScan()
        with self.parent._acq_progress_lock:
            if preset:
    ...
    def _setExternal(self, external):
        logging.debug(f"Setting {DEVICE_LABELS[self._device_handler.device_type]} external mode to {external}")
    ...
    def _pollVAs(self):
        try:
            with self.parent._acquisition_init_lock:
                logging.debug(f"Updating {DEVICE_LABELS[self._device_handler.device_type]} FoV, voltage and current")
...
class Detector(model.Detector):
    ...
    def _onBpp(self, bpp):
        with self.parent._acq_progress_lock:
            self._device_handler.DtEnable(self._channel, 1, bpp)
