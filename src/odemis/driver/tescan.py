...
TESCAN_SHUTTER_TYPE = 1  # Value for the SEM shutter type on all TESCAN systems.
...
class Scanner(model.Emitter):
        ...
        **kwargs
        ...
        if self._device_handler.device_type == DeviceType.ELECTRON:
            ...
            try:
                ...
                shutter_id = self._get_shutter_id_by_type(TESCAN_SHUTTER_TYPE)
                ...
                logging.warning(f"Shutter initialization failed: {e}")
    ...
    def _get_shutter_id_by_type(self, shutter_type: int) -> int:
        ...
        The ShutterEnum() API returns a string like:
        "shutter.0.name=... shutter.0.id=... shutter.0.type=... shutter.1.name=..."
    ...
    def _setBeamPreset(self, preset: str) -> str:
        ...
        if preset:
            self._device_handler.ScStopScan()
            with self.parent._acq_progress_lock:
                # Store scan rotation, dwell time and fov so we can revert it after.
                scan_rotation = self._device_handler.GetImageRot()
                dwell_time = self._device_handler.ScGetSpeed()

                # Set the preset
                self._device_handler.PresetSetEx(preset)

                # Somehow SharkSEM API resets the scan parameters after changing the preset.
                # This is undesired for our use case, so restore it.
                self._device_handler.SetImageRot(scan_rotation)
                self._device_handler.ScSetSpeed(dwell_time)
        return preset
    ...
    def _set_shutter_position(self, value: Union[bool, None]) -> Union[bool, None]:
        ...
        Note: Acquires _acq_progress_lock to prevent polling from overwriting the
    ...
    def terminate(self):
        ...
        # Restore original shutter mode if it was initialized
        if hasattr(self, '_original_shutter_mode') and self._original_shutter_mode is not None:
            try:
                self._device_handler.ShutterSetMode(self._shutter_id, self._original_shutter_mode)
                logging.info(f"Restored shutter mode to {self._original_shutter_mode}")
            except Exception as e:
                logging.error(f"Failed to restore shutter mode: {e}")
