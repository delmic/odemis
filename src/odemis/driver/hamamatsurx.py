...
# HPDTA has a very specific way to report time, so need to convert from float to "HPDTA time str",
# back and forth.

TIME_UNITS = ['s', 'ms', 'us', 'ns']  # Order matters!


def convert_hw_time_to_sec(value: str) -> float:
    """
    Converts a value plus its corresponding unit as received from RemoteEx, to a float.
    :param value: value
    :return: time value in s
    """
    try:
        value, unit, *extra = value.split(" ")
    except (ValueError, AttributeError):
        raise ValueError(f"Unit conversion for value {value} not supported")

    if extra:
        logging.warning("Time value %s has extra parts %s, ignoring them", value, extra)

    try:
        value = float(value) * 10 ** (TIME_UNITS.index(unit) * -3)
    except ValueError:
...
def convert_sec_to_hw_time(value: float) -> str:
    """
    Converts a value to a value plus corresponding unit, which will be accepted by RemoteEx.
    :param value: time duration in seconds
    :return: a string consisting of a value plus unit
    """
    ...
    if 1e-9 <= value < 1:
        ...
        value_raw = str(int(round(value * conversion))) + " " + TIME_UNITS[unit_index]
...
class ReadoutCamera(model.DigitalCamera):
    ...
    def __init__(self, name: str, role: str, parent: model.HwComponent,
        ...
        if can_photon_counting:
            ...
            self.pcIntegrationCounts = model.IntContinuous(integ_count, count_range, unit="count",
            ...
            self.pcThreshold = model.IntContinuous(threshold, threshold_range, unit="count", setter=self._set_pc_threshold)
    ...
    def _set_photon_counting(self, value: bool) -> bool:
        ...
        :param value: True to enable photon-counting, False to disable
    ...
    def _getCamExpTimeRange(self):
        ...
        min_exp = convert_hw_time_to_sec(min_value)
        max_exp = convert_hw_time_to_sec(max_value)
    ...
    def GetCamExpTime(self) -> float:
        ...
        Converts the provided value received from RemoteEx into seconds.
        :return: exposure time in seconds
        """
        exp_time_raw = self.parent.CamParamGet("Live", "Exposure")[0]
        try:
            exp_time = convert_hw_time_to_sec(exp_time_raw)
        except Exception:
    ...
    def _setCamExpTime(self, value: float) -> float:
        ...
        Converts the time range in seconds into a for RemoteEx readable format.
        :param value: exposure time to be set
        :return: current exposure time
        """
        try:
            exp_time_raw = convert_sec_to_hw_time(value)
        except Exception:
    ...
    def _get_pc_exp_time(self) -> float:
        ...
        :return: exposure time in seconds
        """
        exp_time_raw = self.parent.CamParamGet("PC", "Exposure")[0]
        try:
            exp_time = convert_hw_time_to_sec(exp_time_raw)
        except Exception:
            raise IOError("Exposure time of %s failed to be converted to a float" % exp_time_raw)
    ...
    def _set_pc_threshold(self, threshold: int) -> int:
        ...
        :param threshold: threshold in counts
        :return: actual threshold set
        """
    ...
    def _update_monitor_mode(self, active: bool, photon_counting: bool = False) -> None:
        ...
        acquisition mode.
        :param active: True if acquisition is active, False otherwise
    ...
    def _get_image(self, event_args: List[str], acq_start_t: Optional[float], photon_counting: bool) -> model.DataArray:
        ...
        # TODO: it would be better to read the metadata at the acquisition start instead of once it's
        # over, because the user could have just changed the settings. In practice, it's a very rare
        # edge-case, and quite hard to deal with the live mode. So for now, that's a limitation,
        # that probably very few people will ever notice.
        ...
        if photon_counting:
            ...
            # Don't trust .pcIntegrationCounts & .pcExposureTime too much, as the user might have
            # changed them in HPDTA.
            try:
                exposure_count = self._get_exposure_count()
                exp_time_raw = self.parent.CamParamGet("PC", "Exposure")[0]
                exp_time_pc = convert_hw_time_to_sec(exp_time_raw)
...
class StreakUnit(model.HwComponent):
    ...
    def SetTimeRange(self, location, time_range):
        ...
        Converts the value in seconds into a for RemoteEx readable format.
        :param location: (str) see DevParamGet
        :param time_range: (float) time range for one sweep in seconds
        """
        try:
            if self._time_ranges:
                ...
                time_range_raw = convert_sec_to_hw_time(time_range)
    ...
    def GetTimeRangeChoices(self):
        ...
        choices = []
        for choice in choices_raw:
            if " " in choice:  # contains unit (eg, "1.0 ns")
                t = convert_hw_time_to_sec(choice)
            else:  # No unit -> typically it's just a number (from 1 to 5)
                time_id = int(choice)
    ...
    def GetTimeRange(self) -> float:
        ...
        Converts the provided value received from RemoteEx into seconds.
        :return: (float) current time range for one sweep in seconds
        """
        time_range_raw = self.parent.DevParamGet(self.location, "Time Range")[0]
        if " " in time_range_raw:  # contains unit (eg, "1.0 ns")
            time_range = convert_hw_time_to_sec(time_range_raw)
        else:# No unit -> typically it's just a number (from 1 to 5)
            time_id = int(time_range_raw)
    ...
    def get_time_scale_factor(self) -> float:
        ...
        :return: factor to convert time scale values to seconds
        """
...
class StreakCamera(model.HwComponent):
    ...
    def readCommandResponse(self):
        ...
        try:
            ...
            while self.should_listen:
                ...
                # => if the response looks like it could be cut, wait for more
                if len(responses) > 80 or responses[-1:] != "\r" or "\n" in responses:
                    for i in range(100):
                        time.sleep(3e-3)
                        ...
                        try:
                            ...
                            logging.debug("No more data available while expected more: %s", ex)
                            break
                    else:
                ...
                for msg in resp_splitted:
                    ...
                    if error_code in (4, 5):
                        ...
                        if error_code == 4 and rfunc.lower() in ("livemonitor", "acqmonitor"):
                            ...
                            # Discard old warnings if too many
    ...
    def AppStart(self, visible: bool = True, ini_file: Optional[str] = None, no_dialog: bool = True):
        ...
        :param visible: if True, the HPDTA application will be visible. Otherwise, it will run in the background.
        :param ini_file: path to the INI file for HPDTA, default is HDPTA8.INI
