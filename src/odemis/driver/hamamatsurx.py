...
class OrcaFlash(model.DigitalCamera):
    ...
    #             # if self._streakunit.mode.value == "Operate":
...
class StreakUnit(model.HwComponent):
    ...
    def __init__(self, name, role, parent, location, **kwargs):
        ...
        # VAs
        mode = self._convertOutput2Value(self.parent.DevParamGet(location, "Mode"))
        self.mode = model.VigilantAttribute(mode, setter=self._updateMode)
    ...
    def _updateMode(self, value):
        ...
        self.parent.DevParamSet(self.location, "Mode", value)
...
class DelayGenerator(model.HwComponent):
    ...
    def __init__(self, name, role, parent, location, **kwargs):
        ...
        delayA = self._getDelayA()
        range = self._getDelayATimeRange()
        self.delayA = model.FloatContinuous(delayA, range, setter=self._updateDelayA)
    ...
    def _updateDelayA(self, value):
        """
        update the mode VA
        """
        value_str = self._convertInput2Str(value)
        self.parent.DevParamSet(self.location, "Delay A", value_str)
        logging.debug("Reporting delay A %s for delay generator.", value)
        self._metadata[model.MD_DELAY_A] = value

        return value

    def _getDelayATimeRange(self):
        """
    ...
    def _getDelayA(self):
        """Get the value for delay A."""
        delayA_raw = self.parent.DevParamGet(self.location, "Delay A")
        delayA = self._convertOutput2Value(delayA_raw)
