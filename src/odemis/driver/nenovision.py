...
class LiteScope(model.Actuator):
    ...
    def __init__(self, name: str, role: str, axes: Dict[str, Dict[str, Any]], device: str = "Dev1",
        ...
        init_positions = {ax: self._voltage_to_meters(ax, self._voltage[ax]) for ax in axes}
        self.position = model.VigilantAttribute(init_positions, unit="m", readonly=True)

        # Enforce speed limits
        self.speed = model.MultiSpeedVA(
            self._speed,
            [MIN_SPEED_M_S, MAX_SPEED_RETRACTED_M_S],
            unit="m/s",
    ...
    def _meters_to_voltage(self, axis: str, val_m: float) -> float:
        """
    ...
    def _voltage_to_meters(self, axis: str, val_v: float) -> float:
        """
    ...
    def _set_voltage(self, volts: Dict[str, float]) -> None:
        ...
        self.position._set_value(
            {ax: self._voltage_to_meters(ax, self._voltage[ax]) for ax in self.axes},
            force_write=True
        )
    ...
    def _write_analog_output_finite(self, axes: List[str], waveform: numpy.ndarray, sample_rate: float,
    ...
    def _do_move_rel(self, future: CancellableFuture, shift: Dict[str, float]) -> None:
        ...
        :param shift: A mapping of axis names to positional shifts in meters.
        """
        target = {ax: self._voltage_to_meters(ax, self._voltage[ax]) + sh for ax, sh in shift.items()}
    ...
    def _do_move_abs(self, future: CancellableFuture, pos: Dict[str, float]) -> None:
        ...
        target_v = {ax: self._meters_to_voltage(ax, pos[ax]) for ax in axes}
        ...
        samples_generated = self._write_analog_output_finite(axes, waveform, sample_rate, n_samples, duration, future)
