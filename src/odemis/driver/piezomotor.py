...
class PMD401Bus(Actuator):
    ...
    def isMovingClosedLoop(self, axis):
        ...
        if not d3 & 2:  # closed loop not active, thus it is not moving in closed loop
    ...
    def _sendCommand(self, cmd):
        ...
        with self._ser_access:
            ...
            if not resp.endswith(EOL):
                raise IOError("Response %s does not ends with %s, it's due to a timeout" % (to_str_escape(resp), EOL))
