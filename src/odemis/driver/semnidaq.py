    ...
    def _acquire_series(self, detectors: list, continuous: bool) -> bool:
        ...
        with nidaqmx.Task("AO") as ao_task, nidaqmx.Task("DO") as do_task, nidaqmx.Task("AI") as ai_task:
            ...
            if acq_settings.do_samples_n:
                ...
                # One option would be to duplicate the buffer, as we do when AO has only 1 sample.
                # However, another workaround is to just disable the SDK buffer. Maybe we actually
                # could always disable the SDK buffer, but it's not really clear when it's necessary.
                # So, to be safe, we only disable it when the sample size is really tiny.
                if self._do_data is not None and acq_settings.do_samples_n <= 16:
                    logging.debug("Using on-board memory for DO task")
                    for do_chan in do_task.do_channels:
                        do_chan.do_use_only_on_brd_mem = True
            else:
                self._do_task = None
