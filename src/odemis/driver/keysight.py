...
class TrueForm(model.Emitter):
    ...
    def setDutyCycle(self, c: int, dc: float) -> None:
        ...
        """
        # Note: if using a different waveform, the only change required to replace "puls" by the waveform name.
    ...
    def _set_delay(self, d: float) -> float:
        ...
        """
        delay_max = self.period.value * 0.99999
        if not (-delay_max <= d <= delay_max):
            logging.warning("Delay %s is out of bounds for period %s, will clip it", d, self.period.value)
            d = min(max(-delay_max, d), delay_max)
    ...
    def _set_rise_time(self, d: float) -> float:
        """
        Setter for the .riseTime VA
        :param d: rise time (s), must be > 0 and < .period
        :return: accepted rise time (s) by the device
        """
    ...
    def _set_fall_time(self, d: float) -> float:
        """
        Setter for the .fallTime VA
        :param d: fall time (s), must be > 0 and < .period
        :return: accepted fall time (s) by the device
        """
        if d > self.period.value:
    ...
    def _set_duty_cycle(self, dc: float) -> float:
        """
        Setter for the .dutyCycle VA
        :param dc: duty cycle (0-1)
        :return: accepted duty cycle (0-1) by the device
        """
        if not self._is_powered:
            return dc
    ...
    def _power_device_on(self):
        ...
        # Configure the (pulse/square) waveform voltages: low = min and high = max
        ...
        for i in range(3):
            ...
            logging.error("Failed to set the waveform to pulse after 3 attempts: %s", last_ex)
            raise last_ex
        ...
        duty_cycle = self.dutyCycle.value
        period = self.period.value
        delay = self.delay.value
        rise_time = self.riseTime.value
        fall_time = self.fallTime.value
        try:
            self.dutyCycle.value = duty_cycle
        except TrueFormError as ex:
            logging.warning("Failed to set the duty cycle to %s after power on: %s", duty_cycle, ex)
        ...
        for c, t in self._tracking.items():
            ...
            try:
                ...
                logging.warning("Failed to set channel %s to tracking mode %s, will retry: %s", other_c, t, ex)
                self.setTracking(other_c, t)
