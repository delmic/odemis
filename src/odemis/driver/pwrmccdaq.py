...
from odemis.model import Emitter, HwComponent, HwError
...
class MCCDeviceDIOActuator(model.Actuator):
    ...
    def __init__(self, name: str, role: str, parent: MCCDevice, axes: Dict[str, dict], **kwargs):
        ...
        :param axes (dict str -> dict): axis name -> axis configuration dict with:
              ...
              "positions" (dict float -> str): mapping of position values to human-readable names.
                Since it's a binary axis, only two positions should be defined, one for the low position
                and one for the high position.
            Example:
        ...
        for axis_name, axis_cfg in axes.items():
            missing_keys = {"do", "low", "high", "duration", "positions"} - set(axis_cfg.keys())
            if missing_keys:
            ...
            self._axes_cfg[axis_name] = (do_ch, low, high, duration)
            axes_def[axis_name] = model.Axis(unit=unit, choices=positions)
