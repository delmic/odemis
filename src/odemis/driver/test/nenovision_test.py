...
class TestLiteScope(unittest.TestCase):
    ...
    def test_m_to_v_at_limits(self):
        """Physical range limits must map exactly to voltage range limits."""
        for ax in ("x", "y"):
            rng_m = CONFIG_AXES[ax]["rng_m"]
            rng_v = CONFIG_AXES[ax]["rng_v"]
            self.assertAlmostEqual(self.scan_stage._m_to_v(ax, rng_m[0]), rng_v[0], places=10)
            self.assertAlmostEqual(self.scan_stage._m_to_v(ax, rng_m[1]), rng_v[1], places=10)

    def test_v_to_m_at_limits(self):
        """Voltage range limits must map exactly to physical range limits."""
        for ax in ("x", "y"):
            rng_m = CONFIG_AXES[ax]["rng_m"]
            rng_v = CONFIG_AXES[ax]["rng_v"]
            self.assertAlmostEqual(self.scan_stage._v_to_m(ax, rng_v[0]), rng_m[0], places=10)
            self.assertAlmostEqual(self.scan_stage._v_to_m(ax, rng_v[1]), rng_m[1], places=10)

    def test_m_to_v_roundtrip(self):
            ...
            for val_m in [-80e-6, -40e-6, 0.0, 40e-6, 80e-6]:
                with self.subTest(axis=ax, val_m=val_m):
                    self.assertAlmostEqual(
                        self.scan_stage._v_to_m(ax, self.scan_stage._m_to_v(ax, val_m)),
                        val_m,
                        places=15,
                    )
    ...
    def test_speed_va_rejects_above_max(self):
        ...
        over = LiteScope.MAX_SPEED_RETRACTED_M_S * 10
    ...
    def test_speed_va_rejects_below_min(self):
        ...
        under = LiteScope.MIN_SPEED_M_S / 10
        with self.assertRaises(IndexError):
    ...
    def test_scan_full_range_sequential_small_moves(self):
        ...
        self.scan_stage.speed.value = {
            "x": LiteScope.MAX_SPEED_RETRACTED_M_S,
            "y": LiteScope.MAX_SPEED_RETRACTED_M_S,
        }
    ...
    def test_move_start_stop_overhead_full_range(self):
        ...
        for i, target_x in enumerate(positions):
            x_start = self.scan_stage.position.value["x"]

            rng_v = CONFIG_AXES["x"]["rng_v"]
            rng_m = CONFIG_AXES["x"]["rng_m"]
            ...
            overhead = elapsed - theoretical_s
            logging.info(f"Move {i+1}/{len(positions)}: elapsed={elapsed:.4f}s, theoretical={theoretical_s:.4f}s, overhead={overhead*1000:.3f}ms")
    ...
    def test_debug_plot_voltage_curve_0_to_80um(self):
        ...
        x_start_m, x_end_m = 0.0, 80e-6
        x_start_v = {"x": self.scan_stage._m_to_v("x", x_start_m)}
        x_target_v = {"x": self.scan_stage._m_to_v("x", x_end_m)}
        ...
        ax_voltage.set_title(f"X-axis voltage: {x_start_m*1e6:.0f} µm → {x_end_m*1e6:.0f} µm  "
