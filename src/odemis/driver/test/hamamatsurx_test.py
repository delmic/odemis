...
class TestHamamatsurxCam(unittest.TestCase):
    ...
    ### Delay generator #####################################################
    def test_DelayA(self):
        """Test Acquisition Mode VA for delay generator."""
        self.delaybox.delayA.value = 0
        prev_delayA = self.delaybox.delayA.value
        # change delayA VA
        self.delaybox.delayA.value = 0.000001  # 1us
        cur_delayA = self.delaybox.delayA.value
        # check previous and current value are not the same
        self.assertNotEqual(prev_delayA, cur_delayA)
        # get delayA from hardware
        remoteEx_delayA = self.delaybox._convertOutput2Value(
            self.streakcam.DevParamGet(self.delaybox.location, "Delay A"))
        self.assertEqual(cur_delayA, remoteEx_delayA)

        # request value, which is not in range of VA
        with self.assertRaises(IndexError):
            self.delaybox.delayA.value = -1

    ### Streakunit #####################################################
    def test_OperatingMode(self):
        """Test operating mode VA of streak unit."""
        self.streakunit.mode.value = "Focus"
        ...
        # change mode VA
        self.streakunit.mode.value = "Operate"
        remoteEx_mode = self.streakunit._convertOutput2Value(self.streakcam.DevParamGet(self.streakunit.location, "Mode"))
        self.assertEqual(remoteEx_mode, "Operate")
        self.assertEqual(self.streakunit.mode.value, "Operate")
        ...
        self.assertEqual(remoteEx_mode, "Focus")
        self.assertEqual(self.streakunit.mode.value, "Focus")
    ...
    def test_metadataUpdate(self):
        ...
        self.streakunit.mode.value = "Focus"
        self.assertEqual(self.streakunit._metadata[model.MD_STREAK_MODE], "Focus")
        self.streakunit.mode.value = "Operate"
        self.assertEqual(self.streakunit._metadata[model.MD_STREAK_MODE], "Operate")
    ...
    def test_acq_getScalingTable(self):
        ...
        self.assertEqual(img.metadata[model.MD_STREAK_MODE], "Focus")
    ...
    def test_acqSync_SingleLive_RingBuffer_subscribe(self):
        ...
        self.readoutcam.exposureTime.value = 0.2  # s
        ...
        def receive_image(dataflow, image):
            ...
            self.assertEqual(image.metadata[model.MD_STREAK_MODE], "Focus")
            self.images_left -= 1
        ...
        def receive_image(dataflow, image):
            ...
            self.assertEqual(image.metadata[model.MD_STREAK_MODE], "Operate")
            self.images_left -= 1
            if self.images_left == 0:
                dataflow.unsubscribe(receive_image)
                self.end_time = time.time()
