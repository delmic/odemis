...
class MightyEBIC(Detector):
    def __init__(self, name: str, role: str, channel: int, url: str, **kwargs):
        ...
        :param channel (int): The channel from which the device read the data (starts from 0)
        ...
        try:
            self._opc_client = MightyEBICUaClient(self._url, DEFAULT_TIMEOUT)
        except OSError as ex:
            raise HwError(f"Failed to connect to MightyEBIC computer, check connection: {ex}")

        # TODO: any way to support multiple channels? Should we provide multiple DataFlows?
        ...
        dt_max = self._opc_client.calculate_dwell_time(oversampling=max(OVERSAMPLING_SET_VALUES),
    ...
    def _acquire_thread(self) -> None:
        ...
        try:
            ...
            logging.debug("Acquisition started, state = %s, expected scan time = %s s, will wait for up to %s seconds",
                          state, scan_time, DEFAULT_TIMEOUT + scan_time)
            ...
            while self._opc_client.controller_state in (STATE_NAME_BUSY, STATE_NAME_TRIGGER):
                if time.time() > end_time:
                    ...
                    self._opc_client.stop_scan()
                    raise TimeoutError("Acquisition not ended after %s seconds" % (DEFAULT_TIMEOUT + scan_time,))
                if self._acquisition_must_stop.wait(0.1):
                    self._opc_client.stop_scan()
                    return
    ...
    def read_data(self, resolution: Tuple[int, int], channels: int, md: Dict[str, Any]) -> List[model.DataArray]:
        ...
        if result_shape != (resolution[0], resolution[1], channels):
            logging.warning("Expected a result of shape %s, but reported to be %s", resolution, result_shape)
        ...
        das = [model.DataArray(raw_arr[i], md) for i in range(channels)]
        return das
...
class MightyEBICUaClient:
    ...
    def __init__(self, url: str, timeout: float):
        ...
        f = asyncio.run_coroutine_threadsafe(self.client.connect(), self._loop)
        f.result()
    ...
    def guess_samples_per_pixel_and_oversampling(self, req_dt: float, channels: int, delay: float
        ...
        """
        See _guess_samples_per_pixel_and_oversampling() for info
        """
    ...
    async def _guess_samples_per_pixel_and_oversampling(self, req_dt: float, channels: int, delay: float
        ...
        oversampling rate is essentially the same, but it is down at a lower level, and so is more
        efficient, but has a limited set of values possible.
        ...
        """
        # Try every oversampling rate, and for each of them find the best SPP.
        # Pick the best combination of SPP x OS, offering the largest dwell time below the requested dwell time.

        dt_to_params = {}
        for os in sorted(OVERSAMPLING_SET_VALUES):
            dt, spp = await self._guess_samples_per_pixel(req_dt, os, channels, delay)
            dt_to_params[dt] = (spp, os)
            if spp == 1:
                # Increasing the oversampling will not allow to have a shorter dwell time, so stop early
                break

        # Pick the best one: the largest, below the requested dwell time (and the largest OS)
        try:
            best_dt = max(dt for dt in dt_to_params if dt <= req_dt)
            spp, os = dt_to_params[best_dt]
            # If there are several dwell times with the same value for spp * os, pick the largest os
            # (which might be slightly short dwell time, but will be more efficient)
            int_counts = spp * os
            dt_same_counts = [dt for dt, (spp, os) in dt_to_params.items() if spp * os == int_counts]
            best_dt = min(dt_same_counts, key=lambda dt: dt_to_params[dt][1])
            spp, os = dt_to_params[best_dt]
            logging.debug("Best dwell time found for %s: %s, with spp = %s, os = %s", req_dt, best_dt, spp, os)
        except ValueError:  # No dt < req_dt
            best_dt = min(dt_to_params.keys())
            spp, os = dt_to_params[best_dt]
            logging.debug("No dwell time found below the requested %s s, picking %s s with spp = %s, os = %s", req_dt,best_dt, spp, os)

        return best_dt, spp, os
    ...
    async def _guess_samples_per_pixel(self, req_dt: float, oversampling: int, channels: int, delay: float
        ...
        logging.debug("Starting with an estimate of dt = %f (spp = %s, os = %d)",
        ...
        logging.info("Guessing samples per pixel for dt_req = %s us as dt %s = us = %s spp (in %d attempts)",
                     req_dt_us, dt, spp, attempts)
        return dt * 1e-6, spp
    ...
    def calculate_dwell_time(self, oversampling: int, channels: int, spp: int, delay: float) -> float:
        ...
        f = asyncio.run_coroutine_threadsafe(self._calculate_dwell_time(oversampling, channels, spp, delay),
                                             self._loop)
        dt_us = f.result()
        # If some incorrect paramaters are sent, it returns an int <= 0.
        if dt_us <= 0:
    ...
    async def _calculate_dwell_time(self, oversampling: int, channels: int, spp: int, delay: float) -> int:
        ...
        For the parameters, see _start_trigger_scan().
        :return: dwell time in μs
        """
        if not oversampling in OVERSAMPLING_SET_VALUES:
    ...
    def get_scan_result(self) -> List[float]:
        ...
        from the fastest to the slowest. So it is *not* in the same order as it was acquired.
    ...
    def get_scan_result_shape(self) -> Tuple[int, int, int]:
        ...
        :return: p_fast, p_slow, channels pixels of the scan result
        """
...
class MightyEBICSimulator(Server):
    ...
    async def connect_to_server(self):
        ...
        """
        logging.info("Starting simluator OPC-UA server")
        await self.setup()
