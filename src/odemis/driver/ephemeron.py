...
class MightyEBIC(Detector):
    ...
    def read_data(self, resolution: Tuple[int, int], channels: int, md: Dict[str, Any]) -> List[model.DataArray]:
        ...
        logging.debug("Shape da0 = %s", das[0].shape)  #DEBUG
        return das
...
class MightyEBICUaClient:
    ...
    async def _initialize_client(self):
        ...
        # TODO: probably useless: it seems to be just using some sort of polling every 0.5 s. That
        #  is actually much less precise than doing it explicitly when we need. => Drop?
        # Subscribe to receive every (scan) "state" change.
        # For now, only used for logging, but could be used to more quickly detect the end of an acquisition.
