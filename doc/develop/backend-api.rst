...
via the use of this API. It is specifically focused to represents and manipulate
microscope hardware.
...
.. py:class:: HwComponent(name, role[, parent=None][, children=None])
    ...
    .. py:attribute:: role 
        ...
        *(RO, str)* Role (function) of the component in the microscope (the software should used this to locate the different parts of the microscope in case the type is not sufficient: each role is unique in a given model)
...
.. py:class:: Emitter()
    ...
    .. py:attribute:: shape
        ...
        *(1024, 1024)*, while a simple light might have a empty shape of
        *()*.
...
.. py:class:: Light()
    ...
    .. py:attribute:: emissions
        ...
        *(VA, list of 0 <= float <=1)* ListVA which contains one or more entries of relative strength of emission source. The actual wavelength generated by each source is described in the .spectra attribute (e.g., this can be seen as a palette-based  The hardware might or might not be able to generate light from all the entries simultaneously. However, the component should accept all potentially correct values and adapt the value to the actual hardware.
...
.. py:class:: Scanner()
    ...
    .. py:attribute:: dwellTime
        ...
        *(VA of float, optional, unit=s)* How each pixel is scanned.
