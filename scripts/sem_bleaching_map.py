...
run as:
./scripts/sem_bleaching_map.py --dt=600e-6 --dtsem=30e-6, --roi=0.4,0.4,0.95,0.95 --pxs=376 --subpx=6 --lpower=0.3 --lemissions=1 --exptime=1 --output=filename.tiff -olp 0.3 -ole 1 -oet 1 --gridpitch=4 --drift=3 --anchor=0.01,0.01,0.1,0.1

--dt defines the dwell time in seconds when scanning (time required to bleach one spot/sub-pixel)
--dtsem is the required time in seconds to take a SEM image
...
--subpx the number of subpixels (must be a square of an integer).
...
--lemissions gives the emission source (numbered from 0 to n-1).
--exptime gives the exposure time in seconds
-olp is the excitation light power used for overview image
-ole is the emission source (numbered from 0 to n-1) used for overview image
-oet is the exposure time that is used for overview image
--drift defines how ofter drift correction is performed
--gridpitch gives the number of pixels between two consecutive grids
--anchor the top-left and bottom-right points of the region that performs drift correction
...
SIDES = 20  # how many pixels outside the ROI are saved (frame of s CCD pixels width)
BORDER = 0  # how many pixels outside the ROI are bleached (frame of s CCD pixels width (is approximated by a number of EM pixels))

logging.getLogger().setLevel(logging.INFO)  # put "DEBUG" level for more messages


class Acquirer(object):
    def __init__(self, dt, dtsem, roi, pxs, subpx=1, gridpitch=0, lpower=0.2, lemissions=0, exptime=2.0, olpower=None,
                 olemissions=None, oexptime=None, dperiod=None, anchor=None):
        """
        pxs (0<float): distance in nm between center of each tile
        ...
        self.emissions = [0] * len(self.light.emissions.value)
        self.emissions[lemissions] = 1
    ...
    def configure_sem_for_survey(self, roi):
        ...
        b_rel = [b * ccd_pxs[0] / (full_width[0] * sem_pxs[0]),
                 b * ccd_pxs[1] / (full_width[1] * sem_pxs[1])]
    ...
    def configure_and_acquire_sem_survey(self, roi):
        """
    ...
    def check_gridpitch(self, rep, gridpitch):
        """
        Checks whether gridscanning is enabled, and updates rep to be divisble by gridpitch
        """
        if rep[0] < gridpitch[0] or rep[1] < gridpitch[1]:
            logging.warning("Disabling gridscanning because number of SR pixels %s, smaller than gridpitch %s", rep,
                            gridpitch)
            self.gridpitch = (rep[1], rep[0])  # this effectively disables gridscanning
        elif gridpitch[0] > 0 and gridpitch[1] > 0:
            # we need to account for the gridpitch
            # rep must be a multiple of gridpitch
            logging.info("Target nr of SR pixels before gridding: %s", rep)
    ...
    def bleach_borders(self, roi):
        ...
        b_px = [int(max(1, b*ccd_pxs[0] / (ebeam_scale[0]*sem_pxs[0]))),
                int(max(1, b*ccd_pxs[1] / (ebeam_scale[1]*sem_pxs[1])))]  # in px at current scale
    ...
    def bleach_spot(self, pos):
        ...
        # logging.info("SEM coordinates are %s ", self.ebeam.translation.value)
    ...
    def get_fluo_image(self, lpower=None, emissions=None, exptime=None):
        """
        ...
        # Set exposure time and emissions
        self.ccd.exposureTime.value = exptime
        self.light.emissions.value = emissions
    ...
    def acquire_and_save_optical_survey(self, fn, fn_pattern):
        """
        Check whether an overview image is required, then cycle through
        the different settings for exposuretime, emissions and power
        """
        fn_base, fn_ext = os.path.splitext(fn)

        if not self.olpower is None:
            self.ccd.binning.value = (1, 1)
            nemissions = len(self.light.emissions.value)

            for i in range(len(self.olpower)):
                emissions = [0] * nemissions
                emissions[self.olemissions[i]] = 1

                logging.info("Acquiring optical survey nr %i, excitation: %s, power: %fW, exptime: %fs ", i, emissions,
                             self.olpower[i], self.oexptime[i])

                ccd_data = self.get_fluo_image(self.olpower[i], emissions, self.oexptime[i])
    ...
    def acquire(self, fn):
        ...
        try:
            # TODO: Move these constants outside of the class
            s = SIDES
            b = BORDER
            cycles = 1
            ...
            ccd_roi = [ccd_roi[0] - s, ccd_roi[1] - s,
                       ccd_roi[2] + s, ccd_roi[3] + s]  # modification Aaro: increase ROI to capture a border
            ...
            if anchor:
                ...
                self.driftlog = numpy.array([0, 0])
                self.drifttime = numpy.array(time.time())
                self.driftlog.shape = (2, 1)
            ...
            for ii in range(cycles):
                ...
                for g in numpy.ndindex(self.gridpitch):
                    ...
                    # overhead_est = 1 * dur_frame + 0.2
                    overhead_est = 0
                    # overhead_est = 1 * dur_frame + 0.5
                    ...
                    for i in numpy.ndindex(subsetshape):
                        # logging.info("Bleaching pixel (%d,%d) + (%d,%d)*(%d,%d)=(%d,%d)", g[1],g[0],self.gridpitch[1],self.gridpitch[0],i[1], i[0],g[1]+self.gridpitch[1]*i[1],g[0]+self.gridpitch[0]*i[0])
                        sem_data.append(self.bleach_spot(spotssubset[i].tolist()))
                    ...
                    if anchor:
                        ...
                        if n_drift >= next_dc:
                            ...
                            logging.info("Drift estimated to ({:.1f}, {:.1f})".format(*self.drift))
                            n_drift = 0
                            next_dc = px_iter.next()

                            self.save_data(de.raw[-1], "%s%02d-driftAnchor-%05d%s" % (fn_base, ii, n, fn_ext))

                            dlog = numpy.array(self.drift)
                            dlog.shape = (2, 1)

                            self.drifttime = numpy.append(self.drifttime, time.time())
                            self.driftlog = numpy.append(self.driftlog, dlog, 1)
            ...
            # take the SEM image again wiht specified dwell time and step size
            logging.info("Acquiring SEM survey")
            sem_survey_data = self.configure_and_acquire_sem_survey(roi)
            ...
            self.light.power.value = 0  # makes sure light won't stay on
...
def main(args):
    ...
    parser.add_argument("--dtsem", "-ds", dest="dtsem", type=float, required=True,
    ...
    parser.add_argument("--gridpitch", "-g", dest="gridpitch", type=int, default=0,
    ...
    parser.add_argument("--lemissions", "-le", dest="lemissions", type=int, default=0,
                        help="emissions source (numbered from 0 to n-1) used "
                             "for the SR pixels.")
    parser.add_argument("--exptime", "-e", dest="exptime", type=float, default=2.0,
                        help="exposure time used "
                             "for the SR pixels (in seconds).")
    parser.add_argument("--overviewlightpower", "-olp", dest="olpower", type=float, nargs="+", default=[],
                        help="excitation light power used for overview image"
                             "that is acquired before and after the scan.")
    parser.add_argument("--overviewlightemissions", "-ole", dest="olemissions", type=int, nargs="+", default=[],
                        help="emissions source (numbered from 0 to n-1) used "
                             "for overview image that is acquired before and "
                             "after the scan.")
    parser.add_argument("--overviewexposuretime", "-oet", dest="oexptime", type=float, nargs="+", default=[],
    ...
    if not len(options.olpower) == len(options.olemissions) == len(options.oexptime):
        raise ValueError(
    ...
    a = Acquirer(options.dt, options.dtsem, roi, options.pxs * 1e-9, options.subpx, options.gridpitch, options.lpower,
                 options.lemissions, options.exptime, options.olpower, options.olemissions, options.oexptime,
