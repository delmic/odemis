...
class Sparc2AlignmentDataCollectorPlugin(Plugin):
    ...
    def __init__(self, microscope, main_app):
        ...
        self.snapshot_controller = self.main_app._snapshot_controller
...
class AlignmentDataCollectorDialog(wx.Dialog):
    def __init__(
        ...
        # TODO: Enable when not using the snapshot controller to save data
    ...
    def _execute_single_acquisition(
        ...
        self.snapshot_controller.snapshot_viewport(
            self.tab, filepath, self._exporter, anim=False, forcemd={model.MD_EXTRA_SETTINGS: extra_settings_md}
        )
        # TODO: Enable when not using the snapshot controller to save data
    ...
    def _run_acquisition(
        ...
        try:
            ...
            filepath_align = os.path.join(path, "0_snapshot_align_before_acquisition" + hdf5.EXTENSIONS[0])
            ...
            for idx, (l_target, s_target, z_target) in enumerate(acquisition_path):
                ...
                if self._pause_requested:
                    ...
                    # TODO: Enable when a fix is found
                    # self.tab._ccd_stream.is_active.value = False
                    # self.tab._ccd_stream.should_update.value = False
                    # time.sleep(2)
                    # self.tab._spot_stream.is_active.value = True
                ...
                filepath = os.path.join(
                    path, f"{idx}_snapshot_{l_target:.8f}_{s_target:.8f}_{z_target:.8f}" + hdf5.EXTENSIONS[0]
                )
                ...
                if idx in shell_end_indices:
                    ...
                    s0 = self._aligned_pos["s"]
                    z0 = self._aligned_pos["z"]
                    filepath_align = os.path.join(path, f"{idx}_snapshot_align_{l0:.8f}_{s0:.8f}_{z0:.8f}" + hdf5.EXTENSIONS[0])
