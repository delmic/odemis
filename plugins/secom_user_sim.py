...
class SecomUserPlugin(Plugin):
    ...
    def _prepare_gui_for_sim(self, dlg: AcquisitionDialog) -> MicroscopyGUIData:
        """
        Prepares the simulation by switching to the appropriate tab and disabling the buttons.
        :param dlg: The acquisition dialog
        :return: The tab data model
        """
    ...
    def _get_fm_stream(self, tab_data: MicroscopyGUIData) -> FluoStream:
        ...
        :param tab_data: The tab data model
        :return: A FluoStream instance
        :raise LookupError: If no FluoStream is found
        """
        for s in tab_data.streams.value:
            if isinstance(s, FluoStream):
                return s
        raise LookupError("No FluoStream found")
    ...
    def _sim_fluo_search(self, dlg: AcquisitionDialog) -> None:
        ...
        and the binning.
        """
        tab_data = self._prepare_gui_for_sim(dlg)
        try:
            ...
            while not self._should_stop.is_set():
                ...
                if focuser is not None:
                    ...
                    if self._should_stop.wait(1):
                        break
                    # Move back everything to its original position
                    focuser.moveRelSync({"z": -refocus_dist})
