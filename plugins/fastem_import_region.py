...
from odemis.gui.cont.fastem_project_grid import ROAColumnNames, ROIColumnNames
...
class RegionPage(WizardPageSimple):
    """Page to select the region, either ROI or ROA."""
    ...
    def __init__(self, parent):
        ...
        self.region_radio_box = wx.RadioBox(
            panel,
            choices=[RegionType.ROI.value, RegionType.ROA.value],
            majorDimension=1,  # 1 column
            style=wx.RA_SPECIFY_COLS,
        )
        self.region_radio_box.SetSelection(0)  # Default to ROI
...
class ImportRegionPlugin(Plugin):
    ...
    def _create_region_data(
        ...
        :param region_type: Type of the region (ROI or ROA).
        ...
        if region_type == RegionType.ROI.value:
            return self._create_roi_data(
            ...
            sample_type: {
                "projects": {
                    project_name: {
                        ...
                        "rois": regions if region_type == RegionType.ROI.value else [],
                    }
                }
            }
        }
