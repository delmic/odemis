...
class ImportRegionPlugin(Plugin):
    ...
    def _create_regions(self, region_type: str, image_data: dict, labels: numpy.ndarray) -> None:
        ...
        try:
            regions = []
            contours = []
            # loop through unique labels and find contours to ensure each label is a separate region
            for i in numpy.unique(labels):
                temp_labels = labels.copy()
                temp_labels[labels != i] = 0
                contours.extend(find_contours(temp_labels, level=0.5))
