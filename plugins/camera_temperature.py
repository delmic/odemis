...
class CameraTemperaturePlugin(Plugin):
    ...
    def __init__(self, microscope: model.Microscope, main_app) -> None:
        ...
        if MD_TARGET_COOLING_TEMP not in metadata:
            ...
            if target_temp < COOLING_OFF_TEMP:
    ...
    def _on_menu_item(self) -> None:
        ...
        self._dlg = TemperatureControlDialog(self.main_app.main_frame, self._camera)
        self._dlg.Show()
...
class TemperatureControlDialog(wx.Dialog):
    ...
    def __init__(self, parent: wx.Window, camera: model.HwComponent) -> None:
        """
    ...
    def _create_ui(self) -> None:
        ...
        try:
            ...
            min_temp, max_temp = -100, 30

        self._target_spin = wx.SpinCtrlDouble(control_panel, value=str(DEFAULT_COOLING_TEMP),
                                              min=min_temp, max=max_temp, initial=DEFAULT_COOLING_TEMP, inc=1)
    ...
    def _on_target_temperature_update(self, target_temp: float) -> None:
        ...
        try:
            ...
            is_cooling = target_temp < COOLING_OFF_TEMP
    ...
    def _on_cooling_toggle(self, event: wx.Event) -> None:
        ...
        if is_cooling:
            ...
            metadata = self._camera.getMetadata()
            previous_target = metadata.get(MD_TARGET_COOLING_TEMP, DEFAULT_COOLING_TEMP)  # FIXME: should be stored at init, based on what is the current temp
