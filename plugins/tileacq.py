...
class TileAcqPlugin(Plugin):
    ...
    @call_in_wx_main
    def _on_stitch_change(self, stitch: bool):
        """ Show or hide the weaver and register options depending on whether stitching is enabled or not."""
        if self._dlg:
            for entry in self._dlg.setting_controller.entries:
                if hasattr(entry, "vigilattr"):
                    if entry.vigilattr in (self.weaver, self.register):
                        entry.lbl_ctrl.Show(stitch)
                        entry.value_ctrl.Show(stitch)
            self._dlg.setting_controller.panel.Layout()
