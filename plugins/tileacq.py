...
class TileAcqPlugin(Plugin):
    ...
    def _get_region(self, start_pos: dict) -> Optional[Tuple[float, float, float, float]]:
        """
        ...
        :return: (xmin, ymin, xmax, ymax) or None if cannot be determined
        """
        try:
            sfov = self._guess_smallest_fov()
        except ValueError:
            logging.error("Cannot determine FoV for tile acquisition")
            return None
