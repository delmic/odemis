...
# * sets the spectrum stream light power to 0, and disable the entry. So it's impossible to turn it on.
# * when a new spectrum stream is added, it disables the power entry.
# If the mirror is engaged, the power entry of all spectrum streams is re-enabled.
...
class SpectrumLaserProtectionPlugin(Plugin):
    ...
    def __init__(self, microscope, main_app):
        ...
        if microscope and main_data.role.startswith("sparc"):
            ...
            self._stb_ctrl = self._tab.streambar_controller
